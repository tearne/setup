...
if [ "$in_container" = true ]; then
    if ! unshare --user --pid --fork --mount-proc true 2>/dev/null; then
        echo "Error: Running inside a container without nesting support."
        echo "Set 'security.nesting=true' on the outer container and restart it."
        exit 1
    fi
    echo "NOTE: Running inside a container, but nesting appears to work."
fi
